	...
	public function execute(CommandSender $sender, string $commandLabel, array $args): void{
		...
		switch ($args[0] ?? "") {
			...
			case "run":
			{
				...
				(new AStar($world, $this->positions[1], $this->positions[2], $player->getBoundingBox(), (new AlgorithmSettings())->setTimeout(5)->setFallDistance(5)->setOnlyAcceptFullPath(true)->setMaxTicks(0)))->then(function (?PathResult $pathResult) use ($sender, $world): void{
					if (($sender instanceof Player && !$sender->isConnected()) || !$world->isLoaded())
						return;
					if ($pathResult === null) {
					...
					foreach ($pathResult->getPathPoints() as $pathPoint) {
						if (!$world->isChunkLoaded($pathPoint->getFloorX() >> 4, $pathPoint->getFloorZ() >> 4))
							continue;
			...
			case "entity":
			{
				$entity = new TestEntity($this->positions[0] ?? $player->getLocation());
				$entity->spawnToAll();
				break;
			}
			default:
